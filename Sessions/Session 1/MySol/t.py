...
def main():
    ...
    # Check that the two methods (sol and mine) give the same results (K matrices)
    for N in [4, 6, 10, 20]:
        print(f"Running recursion for N = {N}.")
        _, Ksol = riccati_recursion(A, B, R, Q, Pf, N)

        K = lqr_backward(A, B, Q, R, N)

        for _ in range(N):
            print(" ", Ksol[_][0], "vs", K[_])
            if not np.allclose(Ksol[_][0], K[_], atol=1e-8):
                print("Sono diversi")
                break
