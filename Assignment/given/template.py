...
class MPCController:
    ...
    def __init__(self, N: int, dm: string, ts: float, *, params: VehicleParameters):
        """Constructor.
        ...
        self.N = N
        self.ts = ts 
        nlp_dict, self.bounds = self.build_ocp(dm, params)
    ...
    def build_ocp(self, discretization_method, params: VehicleParameters) -> Tuple[dict, dict]:
        """
        TODO IMPLEMENT
...
def exercise3(discr_method):
    ...
    controller = MPCController(N=N, ts=ts, dm=discr_method, params=VehicleParameters())
...
def exercise4(discr_method):
    ...
    # Reduce friction
    params_less_friction = VehicleParameters()
    params_less_friction.friction = 0.8

    print("--Set up the MPC controller")
    controller = MPCController(N=N, ts=ts, dm=discr_method, params=params_less_friction)
    ...
    # Build the assumed model 
    bicycle = KinematicBicycle(params_less_friction)
