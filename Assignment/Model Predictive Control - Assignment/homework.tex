...
\end{assignment}
\begin{flushleft}
	The solver does not converge because the Hessian of the cost is not positive definite. This is proven by a check performed in the code which will print in yellow "Warning: Qk is not positive definite!".
	Figure \ref{fig:assignment64-parking} shows the trajectory of the car, it is obvius that the orgin, parking spot, is not reached.
\end{flushleft}
\begin{figure}[H]
	\centering
...
\end{assignment}
\begin{flushleft}
	With the regularization implemented, the solver converges in 57 iterations. The final solution is shown in Figure \ref{fig:assignment65-states} and the trajectory followed by the car to reach the parking spot is shown in \ref{fig:assignment65-parking}.
\end{flushleft}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{images/Assignment65-regTrue-maxiter70.pdf}
	\caption{Final solution of the parking case using regularization.}
	\label{fig:assignment65-states}
\end{figure}
\begin{figure}[H]
	\centering
